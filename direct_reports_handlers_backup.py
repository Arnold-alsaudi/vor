...
class DirectReportsHandlers:
    ...
    **أمثلة:**
    ...
    🎯 **نوع المخالفة:** {message_reporter.report_type_names.get(report_type, 'غير محدد')}
                ...
                if results.get('reports_per_message'):
                    result_text += f"\n\n📋 **تفاصيل البلاغات لكل رسالة:**\n"
                    for link, stats in list(results['reports_per_message'].items())[:5]:  # أول 5 رسائل
                        success_rate_msg = (stats['successful'] / max(stats['requested'], 1)) * 100
                        result_text += f"• {link.split('/')[-2]}/{link.split('/')[-1]}: {stats['successful']}/{stats['requested']} ({success_rate_msg:.1f}%)\n"
                    
                    if len(results['reports_per_message']) > 5:
                        result_text += f"• ... و {len(results['reports_per_message']) - 5} رسالة أخرى\n"
                ...
                if results['session_stats']:
                    ...
                    for session_id, stats in results['session_stats'].items():
                        ...
                        result_text += f"• {session_id}: {stats['success']}/{total} ({session_rate:.1f}%)\n"
                ...
                # حفظ سجل البلاغات
                log_file = message_reporter.save_report_log(results, report_type, reason)
                if log_file:
                    result_text += f"\n📄 **تم حفظ السجل:** `{log_file}`"
                
                buttons = [
                    [Button.inline("🎯 بلاغات مباشرة جديدة", "direct_message_reports")],
                ...
                error_text = f"❌ خطأ في تنفيذ البلاغات المباشرة: {str(e)}"
                buttons = [
                    [Button.inline("🔄 المحاولة مرة أخرى", "start_direct_reporting")],
                    [Button.inline("🔙 القائمة الرئيسية", "back_to_main")]
                ]
